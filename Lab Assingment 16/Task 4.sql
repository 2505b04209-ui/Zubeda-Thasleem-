...
-- Task 4 Part 1: Mark a book as NOT available when borrowed
UPDATE Books 
SET available = FALSE 
WHERE book_id = 101;

-- Check that it worked
-- Task 4 Part 2: Safely delete a member who has no loans
DELETE FROM Loans WHERE member_id = 3;      -- (no loans, so does nothing)
DELETE FROM Members WHERE member_id = 3;    -- removes Aman Verma

-- Check that member is gone
SELECT * FROM Members;
SELECT * FROM Books;